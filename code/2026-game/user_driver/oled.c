...
void OLED_ShowString(u8 x,u8 y,u8 *chr,u8 size1)
{
	while((*chr>=' ')&&(*chr<='~'))
	{
		...
		x+=size1/2;
		/*
		 * 每个字符的宽度是 size1/2，而不是 size1。
		 * 原判断在 16 号字体下把 128 像素屏幕误算成每行只能显示 15 个字符，
		 * 导致第 16 个字符提前换行。修正后 16 号字体可完整显示 16 个字符。
		 */
		if(x>128-size1/2)  //下一字符放不下时换行
		{
			x=0;
