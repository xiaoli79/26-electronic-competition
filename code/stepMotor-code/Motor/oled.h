...
/**
 * @brief 刷新显存中指定矩形区域所在的 OLED 页。
 *
 * @note SSD1306 以 8 像素为一页传输，传入区域会自动按页对齐。
 */
void OLED_RefreshArea(u8 x, u8 y, u8 width, u8 height);

/**
 * @brief 清除显存中的指定矩形区域，但不立即发送到 OLED。
 *
 * @note 通常在本函数后绘制新文本，再调用 OLED_RefreshArea()。
 */
void OLED_ClearArea(u8 x, u8 y, u8 width, u8 height);

/** @brief 返回 OLED I2C 最近一次传输是否正常 */
uint8_t OLED_IsHealthy(void);

/** @brief 返回 OLED I2C 累计错误次数 */
uint32_t OLED_GetErrorCount(void);

/** @brief 复位 I2C 外设并重新初始化 OLED，成功返回 1 */
